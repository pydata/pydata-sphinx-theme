...
#. In ``conf.py``, add the relative path from your source directory to the custom javascript file:
   ...
   Also ensure your ``custom-icons.js`` ends with ``FontAwesome.dom.i2svg();`` so the
   icons are rendered after being added to the library:

   .. code:: javascript

      FontAwesome.library.add( /* ... your icons ... */ );
