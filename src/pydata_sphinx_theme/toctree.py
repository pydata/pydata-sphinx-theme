...
def _get_ancestor_pagename(app: Sphinx, pagename: str, startdepth: int) -> str:
    """
    Get the name of `pagename`'s ancestor that is rooted `startdepth`
    levels below the global root.
    """
...
def add_toctree_functions(
    ...
    def unique_html_id(base_id: str):
        """
        Create an id that is unique from other ids created by this function
        at build time.
    ...
    def generate_header_nav_html(
        ...
        Parameters:
            n_links_before_dropdown:The number of links to show before nesting
                the remaining links in a Dropdown element.
    ...
    def generate_toctree_html(
        ...
        modified to generate TocTrees for *second*-level pages and below
        (not supported by default in Sphinx).
        ...
        Arguments are passed to Sphinx "toctree" function
        (context["toctree"] below).

        ref: https://www.sphinx-doc.org/en/master/templating.html#toctree

        Parameters:
            kind : "sidebar" or "raw". Whether to generate HTML meant for
            sidebar navigation ("sidebar") or to return the raw BeautifulSoup
                object ("raw").
            startdepth : The level of the toctree at which to start.
                By default, for the navbar uses the normal toctree
                (`startdepth=0`), and for the sidebar starts from the second
                level (`startdepth=1`).
            show_nav_level : The level of the navigation bar to toggle as
                visible on page load. By default, this level is 1, and only
                top-level pages are shown, with drop-boxes to reveal children.
                Increasing `show_nav_level` will show child levels as well.
            kwargs : passed to the Sphinx `toctree` template function.
    ...
    for element in soup.find_all("li", recursive=True):
        ...
        span = soup.new_tag(
            ...
            attrs={
                ...
                # This element and the chevron it contains are purely decorative; # noqa: E501
                # the actual expand/collapse functionality is delegated to the <summary> tag # noqa: E501
                "role": "presentation",
            },
        )
        ...
        if collapsible_section_heading:
            # Put heading inside summary so that the heading text (and chevron)
            # are both clickable
