...
def test_subset_fonts_reduces_size(sphinx_build_factory) -> None:
    ...
    # JS file must exist and contain SVG data for both icon_links icons
    user_icons_js = outdir / "_static/scripts/fontawesome-user-icons.js"
    assert user_icons_js.exists()
    js = user_icons_js.read_text()
    assert 'iconName:"bell"' in js
    assert 'iconName:"github"' in js
    assert 'iconName:"bars"' in js  # PST template icon
